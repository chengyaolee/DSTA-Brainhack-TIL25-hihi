...
def print_differences(truth: str, hypothesis: str):
...
def main():
    ...
    for batch in tqdm(batch_generator, total=math.ceil(len(instances_30) / BATCH_SIZE)):
        response = requests.post("http://localhost:5005/asr", data=json.dumps({
            "instances": batch,
        }))
    ...
    for i in zip(ground_truths, results):
        print("\n\nActual: ",  i[0])
        print("Predicted: ", i[1])
        print("Differences: ", print_differences(i[0], i[1]))
    print("1 - WER:", score)
