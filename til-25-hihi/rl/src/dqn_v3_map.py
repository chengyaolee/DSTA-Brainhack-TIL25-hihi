...
# MODIFIED: Added internal map features (16*16=256) to the input
INPUT_FEATURES_MAP = MAP_SIZE_X * MAP_SIZE_Y
INPUT_FEATURES = 1128 + INPUT_FEATURES_MAP # 7*5*8 (viewcone) * 4 (stack size) + 4 (direction) + 2 (location) + 1 (scout) + 1 (step) + 256 (map)
HIDDEN_LAYER_1_SIZE = 512 # Increased size to handle more features
HIDDEN_LAYER_2_SIZE = 512 # Increased size to handle more features
HIDDEN_LAYER_3_SIZE = 384
...
class TrainableRLAgent:
    ...
    # Update the agent's internal map based on its viewcone
    def update_internal_map(self, internal_map, observation_dict):
        """
        Updates the internal map based on the agent's current viewcone observation.
        Map values: 0.0=Unknown, 0.5=Explored-Empty, 1.0=Explored-Wall.
        """
        location = observation_dict.get("location")
        direction = observation_dict.get("direction")
        viewcone = observation_dict.get("viewcone")

        if location is None or direction is None or viewcone is None:
            return internal_map

        gx, gy = location[0], location[1]
        updated_map = internal_map.copy()

        # Convert viewcone to numpy array if it's a list
        if isinstance(viewcone, list):
            viewcone = np.array(viewcone)

        # The agent's position is the base of its viewcone at row 6, col 2
        for vr in range(viewcone.shape[0]):      # viewcone row, 0-6
            for vc in range(viewcone.shape[1]):  # viewcone col, 0-4
                # Relative coordinates from the agent's perspective in the viewcone
                # (dr, dc): (row_dist_from_agent, col_dist_from_agent)
                # dr is distance in front, dc is distance sideways
                dr = (viewcone.shape[0] - 1) - vr 
                dc = vc - (viewcone.shape[1] // 2)

                # Rotate relative coordinates to global map coordinates
                if direction == 0: # North (-Y)
                    map_x, map_y = gx + dc, gy - dr
                elif direction == 1: # East (+X)
                    map_x, map_y = gx + dr, gy + dc
                ...
                if 0 <= map_x < MAP_SIZE_X and 0 <= map_y < MAP_SIZE_Y:
                    tile_value = viewcone[vr, vc]
                    is_wall = (tile_value & 0b1) == 1
                    map_val = 1.0 if is_wall else 0.5 # 1.0 for wall, 0.5 for empty
                    updated_map[int(map_y), int(map_x)] = map_val
        
        return updated_map

    # MODIFIED: process_observation now takes and uses the internal_map
    def process_observation(self, observation_dict, internal_map):
        processed_features = []
        ...
        if stacked_viewcone is not None:
            ...
            viewcone = observation_dict.get("viewcone", np.zeros((7,5), dtype=np.uint8))
            single_frame_features_len = 7 * 5 * 8
            temp_features = []
            for r in range(7):
                for c in range(5):
                    tile_value = viewcone[r][c] if r < len(viewcone) and c < len(viewcone[r]) else 0
                    temp_features.extend(self._unpack_viewcone_tile(tile_value))
            # Replicate the single frame across the stack size
            for _ in range(STACK_SIZE):
                processed_features.extend(temp_features)
        ...
        # NEW: Add flattened internal map to features
        if internal_map is not None:
            processed_features.extend(internal_map.flatten().tolist())
        ...
        if len(processed_features) != INPUT_FEATURES:
            error_message = (
                ...
                f"Global STACK_SIZE: {STACK_SIZE}. Map shape: {internal_map.shape if internal_map is not None else 'None'}"
            )
...
# --- Worker Function ---
# MODIFIED: Worker now manages internal maps for each agent per episode.
def run_episode_worker(
    ...
    while not stop_event.is_set():
        ...
        # NEW: Keep track of internal maps for each agent in the episode
        agent_internal_maps = defaultdict(lambda: np.zeros((MAP_SIZE_Y, MAP_SIZE_X), dtype=np.float32))

        current_rewards_this_episode = {ag_id: 0 for ag_id in env.possible_agents}
        pending_step_data_scout = {}
        pending_step_data_guard = {}
        
        visited_guard_locations = defaultdict(set)
        ...
        for agent_id in env.agent_iter():
            ...
            current_agent_map = agent_internal_maps[agent_id]
            ...
            if agent_id in pending_this_agent_step_data:
                ...
                if not done and observation is not None:
                    ...
                    try:
                        # NEW: Update map with new observation before creating state representation
                        updated_map = policy_agent_for_action.update_internal_map(current_agent_map, obs_dict_current)
                        agent_internal_maps[agent_id] = updated_map
                        next_state_np = policy_agent_for_action.process_observation(obs_dict_current, updated_map)
            ...
            if not done and observation is not None:
                ...
                try:
                    # NEW: Update map before processing observation for action selection
                    updated_map = policy_agent_for_action.update_internal_map(current_agent_map, obs_dict_current)
                    agent_internal_maps[agent_id] = updated_map
                    current_state_np = policy_agent_for_action.process_observation(obs_dict_current, updated_map)
        ...
        if should_record_video_worker and video_config_worker['video_folder'] and episode_frames:
            ...
            try:
                ...
                # print(f"Worker {worker_id} saved video: {video_path}") # Optional: can be noisy
            except Exception as e:
...
if __name__ == '__main__':
    ...
    try:
        train_parallel(
            ...
            num_episodes_total=1000000,
            num_workers=max(1, mp.cpu_count() -1),
            novice_track=False,
            scout_model_load_path=None,
            save_scout_model_to="map_scout_1m_eps.pth",
            guard_model_load_path=None,
            save_guard_model_to="map_guard_1m_eps.pth",
            render_mode="rgb_array",
            video_folder="./rl_renders_map",
            video_record_interval=1000,
            model_broadcast_learning_steps=TARGET_UPDATE_EVERY * 2
        )
    except ImportError:
