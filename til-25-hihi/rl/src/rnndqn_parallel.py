            ...
            for _ in range(BATCH_SIZE * num_workers): # Process a limited number of items per cycle
                try:
                    ...
                    if agent_type == 'scout':
                        ...
                        if global_episode_completed_counter.value % 10 == 0: # Log less frequently
                        ...
                        if global_episode_completed_counter.value % 100 == 0:
                            print(f"\rEps: {global_episode_completed_counter.value}/{num_episodes_total} | "
                                  f"S_AvgS: {avg_s_score:.1f} | G_AvgS: {avg_g_score:.1f} | Eps: {current_epsilon:.4f} | "
                                  f"S_Buf: {len(scout_learner.memory)} | G_Buf: {len(guard_learner.memory)} | "
...
if __name__ == '__main__':
    ...
    try:
        ...
        train_parallel(
            ...
            num_episodes_total=50000, # Total episodes across all workers
            num_workers=max(1, mp.cpu_count() -2), # Example: Use all but one CPU core
            # num_workers=2, # Or set a fixed number for testing
            novice_track=False,
            scout_model_load_path="agent03_stacked_merged_50k_eps.pth",
            save_scout_model_to="agent03_parallel_scout_100k_eps.pth",
            guard_model_load_path="guard03_stacked_merged_50k_eps.pth",
            save_guard_model_to="agent03_parallel_guard_100k_eps.pth",
            render_mode="rgb_array",
            video_folder="./rl_renders_parallel_merged",
            video_record_interval=200, # Record video every X global episodes
